...
public class ExcelUtil {
    ...
    public static <T> void listToExcel(
        ...
        response.setHeader("Content-disposition", "attachment; filename=" + sheetName + fileName + ".xls");

        //创建工作簿并发送到浏览器
        try {
            sheetName = new String(sheetName.getBytes("ISO8859-1"), "utf-8");

            OutputStream out = response.getOutputStream();
            listToExcel(list, fieldMap, sheetName + fileName+ ".xls", sheetSize, out);
    ...
    private static <T> void fillSheet(
        ...
        for (int i = 0; i < cnFields.length; i++) {
            Label label = new Label(i, 0, cnFields[i], getBodyCellStyle());
            sheet.addCell(label);
        }
        ...
        for (int index = firstIndex; index <= lastIndex; index++) {
            ...
            for (int i = 0; i < enFields.length; i++) {
                ...
                Label label = new Label(i, rowNo, fieldValue,getBodyCellStyle());
                sheet.addCell(label);
            }
